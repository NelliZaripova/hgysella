...
/*   Updated: 2019/08/02 21:23:40 by jwinthei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
									...
									uint8_t i_champ, uint8_t mod)
{
	...
	size = (head[j + 3] & 0x000000FF) << 0 |\
			(head[j + 2] & 0xFF) << 8 | (head[j + 1] & 0xFF) << 16 |\
				(head[j] & 0xFF) << 24;
	if (size)
		cw_out(cw, ft_printf("%w\033[1;31mMissing separator in %s's file%s",\
				STDERR, cw->champ[i_champ]->head.prog_name, "\033[0m\n"), 0);
	if (!mod)
		return ;
	j += DELIMETR_SIZE;
	...
	if (size != cw->champ[i_champ]->head.prog_size)
		cw_out(cw, ft_printf("%w\033[1;31mChampion '%s' %s\033[0m\n",\
			STDERR, cw->champ[i_champ]->head.prog_name,\
			"has a code size that differ from what it's header says"), 0);
}
															...
															uint8_t i_champ)
{
	...
	if (cw->champ[i_champ]->head.magic != COREWAR_EXEC_MAGIC)
		cw_out(cw, ft_printf("%w\033[1;31mInvalid file type\033[0m\n",\
															STDERR), 0);
	k = -1;
	...
	if (!(cw->champ[i_champ]->exec = (uint8_t*)malloc(sizeof(uint8_t) *\
		...
		cw_out(cw, ft_printf("%w\033[1;31mChamp code malloc error\033[0m\n",\
												STDERR), 0);
...
static void			sort_champs(t_cw *cw)
{
	...
	id = 1;
	i = 0;
	while (i < cw->num_of_champs && id <= MAX_PLAYERS)
		if (!cw->champ[i++]->id)
		{
			j = 0;
			while (j < cw->num_of_champs && id <= MAX_PLAYERS)
				if (cw->champ[j++]->id == id && !(j = 0))
					id++;
			cw->champ[i - 1]->id = id;
		}
	i = 0;
