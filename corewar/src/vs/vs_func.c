...
/*   Updated: 2019/08/02 16:24:16 by jwinthei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		vs_backlight_car(t_cw *cw, size_t i_car, int32_t step, uint8_t mode)
{
	...
	pc = cw->car[i_car]->pc;
	if (!(cw->map[pc].v.car = IN(cw->map[pc].v.car)))
	{
	...
	if (mode)
		vs_backlight_on_car(cw, cw->car[i_car]->id_champ, PCV(pc + step), 0);
}
